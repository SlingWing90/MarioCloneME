...
{
    class Brick: BKGObject
    {
        int[] _coordinates;
        int _blockCountX;
        int _blockCountY;

        public Brick(World world, Vector2 position, float width, float height, Texture2D texture):base(world, position, width, height, texture) {
        ...
        public void loadTileset(int lvlID) {
            switch (lvlID) { 
                case 0:
                    this._coordinates[0] = 198;
                    this._coordinates[1] = 259;
                    break;
            }
        }
